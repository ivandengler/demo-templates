...
export default function BarberoDemo({ nombre, direccion, telefono }: Props) {
  ...
  const telDisplay = telefono.length >= 13
    ? `+${telefono.slice(0, 2)} ${telefono.slice(2, 3)} ${telefono.slice(3, 6)} ${telefono.slice(6, 9)}-${telefono.slice(9)}`
    : `+${telefono}`
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-[#111]/95 backdrop-blur-md border-b border-white/5">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-white font-bold tracking-widest text-sm uppercase">{nombre}</div>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 w-full">
          ...
          >
            ...
            <p className="text-white/60 text-lg leading-relaxed mb-10 max-w-lg">
              Servicios de barbería premium en Rosario. Técnica, precisión y una experiencia que vas a querer repetir.
            </p>
            ...
            <div>
              ...
              {step === 'form' && (
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  />
                  <input
                    required type="tel" placeholder="WhatsApp / Teléfono"
                  ...
                  <button
                    type="submit" disabled={!form.time}
              ...
              {step === 'confirm' && (
                ...
                >
                  ...
                  <div className="flex gap-3">
                    <button onClick={() => setStep('done')} className="flex-1 bg-[#C9A84C] text-black font-bold py-3 rounded-full hover:bg-[#b8953e] transition-colors">Confirmar</button>
                    <button onClick={() => setStep('form')} className="flex-1 border border-white/10 text-white/60 py-3 rounded-full hover:border-white/30 transition-colors">Editar</button>
                  </div>
                </motion.div>
              )}
              ...
              <div className="bg-white/3 border border-white/8 rounded-2xl p-6">
                ...
                <div className="space-y-3 text-sm text-white/60">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <a href={`tel:+${telefono}`} className="hover:text-white transition-colors">{telDisplay}</a>
                  </div>
                  <div className="flex items-center gap-3">
                    <Globe className="w-4 h-4 text-[#C9A84C] flex-shrink-0" />
                    <span>@{nombre.toLowerCase().replace(/\s+/g, '').replace(/[^a-z0-9]/g, '')}.rosario</span>
                  </div>
                </div>
              </div>
      ...
      <footer className="border-t border-white/5 py-10">
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-4 text-white/30 text-sm">
          <div className="flex items-center gap-2">
            ...
            <span className="text-white/60 font-semibold tracking-widest uppercase text-xs">{nombre} · Premium</span>
          </div>
