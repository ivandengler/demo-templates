...
export default function InfoDemo({ nombre }: { nombre: string }) {
    ...
    <div className="font-sans bg-white text-zinc-900">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-[#0B2040]/90 backdrop-blur-md border-b border-white/10">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-white font-bold text-lg tracking-tight">{nombre}</span>
          </div>
          <div className="hidden md:flex items-center gap-8 text-sm text-white/70">
            <a href="#destino" className="hover:text-white transition-colors">Nosotros</a>
            ...
            <a href="#contacto" className="bg-cyan-500 text-white px-4 py-2 rounded-full hover:bg-cyan-400 transition-colors font-medium">Contactar</a>
          </div>
        </div>
      </nav>
      ...
      <section className="relative h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center text-white px-6 max-w-4xl mx-auto">
          ...
          >
            Profesionalismo, experiencia y dedicación al servicio de nuestros clientes. Encontranos en Rosario.
          </motion.p>
          <motion.div
      ...
      <section className="bg-[#0B2040] py-12">
        <div className="max-w-4xl mx-auto px-6 grid grid-cols-2 md:grid-cols-4 gap-8 text-center text-white">
          {[
            ...
            { value: "Rosario", label: "Santa Fe, Argentina" },
          ].map((stat) => (
            <div key={stat.label}>
      ...
      <section id="destino" className="py-24 bg-zinc-50">
        <div className="max-w-6xl mx-auto px-6">
          ...
          >
            <p className="text-cyan-600 text-sm uppercase tracking-[0.3em] mb-3">Quiénes somos</p>
            <h2 className="text-4xl font-bold mb-4">¿Por qué elegirnos?</h2>
            <p className="text-zinc-500 max-w-xl mx-auto">Años de experiencia nos respaldan. Nuestro equipo está comprometido con la excelencia y la atención personalizada en cada caso.</p>
          </motion.div>
          <div className="grid md:grid-cols-3 gap-8">
            {[
              { icon: <Waves className="w-8 h-8 text-cyan-500" />, title: "Experiencia comprobada", desc: "Más de una década brindando servicios de calidad en Rosario, con cientos de clientes satisfechos que nos recomiendan." },
              { icon: <Sun className="w-8 h-8 text-cyan-500" />, title: "Atención personalizada", desc: "Cada cliente es único. Analizamos cada situación en detalle para ofrecer la mejor solución posible." },
              { icon: <Users className="w-8 h-8 text-cyan-500" />, title: "Equipo profesional", desc: "Profesionales altamente capacitados, siempre actualizados con las últimas técnicas y estándares del mercado." },
              { icon: <Wind className="w-8 h-8 text-cyan-500" />, title: "Resultados reales", desc: "Nos enfocamos en generar resultados concretos y medibles para nuestros clientes, con total transparencia." },
              { icon: <Star className="w-8 h-8 text-cyan-500" />, title: "Calidad garantizada", desc: "Utilizamos los mejores materiales y métodos disponibles. Tu satisfacción es nuestra prioridad número uno." },
              { icon: <Phone className="w-8 h-8 text-cyan-500" />, title: "Disponibilidad total", desc: "Estamos disponibles para atender tus consultas y necesidades. Comunicación directa y respuestas rápidas." },
            ].map((feat, i) => (
              <motion.div
                key={feat.title}
      ...
      <section id="paquetes" className="py-24 bg-[#0B2040]">
        <div className="max-w-6xl mx-auto px-6">
          <motion.div
            ...
            className="text-center mb-16"
          >
            <p className="text-cyan-400 text-sm uppercase tracking-[0.3em] mb-3">Lo que ofrecemos</p>
            <h2 className="text-4xl font-bold text-white mb-4">Nuestros servicios</h2>
            <p className="text-white/50 max-w-xl mx-auto">Contamos con diferentes opciones para adaptarnos a tus necesidades y presupuesto.</p>
          </motion.div>
          <div className="grid md:grid-cols-3 gap-6">
            {packages.map((pkg, i) => (
              ...
              >
                ...
                <p className={`text-sm mb-6 ${pkg.featured ? 'text-white/70' : 'text-white/50'}`}>{pkg.days} días de servicio</p>
                <div className="mb-6">
      ...
      <section id="galeria" className="py-24 bg-white">
        <div className="max-w-6xl mx-auto px-6">
          ...
          >
            <p className="text-cyan-600 text-sm uppercase tracking-[0.3em] mb-3">Nuestro trabajo</p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {gallery.map((src, i) => (
              ...
              >
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={src} alt={`Foto ${i + 1}`} className="w-full h-full object-cover aspect-square hover:scale-105 transition-transform duration-500" />
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section id="contacto" className="py-24 bg-[#0B2040]">
        <div className="max-w-2xl mx-auto px-6">
          <motion.div
            ...
            className="text-center mb-12"
          >
            <p className="text-cyan-400 text-sm uppercase tracking-[0.3em] mb-3">Contacto</p>
            <h2 className="text-4xl font-bold text-white mb-4">¿Hablamos?</h2>
            <p className="text-white/50">Completá el formulario y te contactamos en menos de 24hs.</p>
          </motion.div>
          {formSent ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <textarea
                rows={4} placeholder="¿En qué te podemos ayudar?"
              ...
              />
              <button type="submit" className="w-full bg-cyan-500 hover:bg-cyan-400 text-white font-semibold py-4 rounded-full transition-all hover:scale-[1.02]">
                Enviar consulta
              </button>
            </form>
          )}
      ...
      <footer className="bg-[#070F1E] py-12 text-white/40 text-sm">
        <div className="max-w-6xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-white/70 font-semibold">{nombre}</span>
          </div>
